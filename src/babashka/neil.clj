...
(defn do-dep-upgrade
  ...
  (when (or (and sha (not= (:git/sha current) sha))
    ...
    (if (:dry-run opts)
      (if sha
        (println :lib lib :version (:git/sha current) :latest-sha sha)
        (println :lib lib :version (:mvn/version current) :latest-version version))
...
dep
  ...
  upgrade: Upgrade all libs in the deps.edn file.
    ...
    Ex: `neil dep upgrade` to upgrade all deps.
    Ex: `neil dep upgrade --dry-run` print deps that would be upgraded.
    Ex: `neil dep upgrade :lib clj-kondo/clj-kondo` to update a single dep.
